...
> ℹ️ **更新通知**：
> 1. コンソールマクロコマンド機能の改善
> 2. 画像をインポートして「フィット」をクリックした後に画像がレンダリングできない問題を修正
> 3. CNCワークフローでXYZ座標をクリックして原点をオフセット設定する機能を追加、絶対位置と相対位置を含む
> 4. VC_redist.x64.exeランタイムライブラリインストーラーを追加
> 5. ハイトマップ適用後にnull値が生成される問題を修正
> 6. Macでのファームウェア書き込み切断の問題を修正
> 7. CAM機能は現在も改善中です、関連するCAM機能はソフトウェアの最終的な効果を表すものではありません
> {.is-info}
