...
回転軸自体の高さが比較的高いため、主軸を上げる必要がある場合があります。主軸の4つの固定ネジを緩め、上に移動させてから再固定します。

<img src="/eng/himill-d1-d1s/media/hmd1s-rotation-axis-processing/2-上调主轴.webp" alt="主軸を上げる" style="width: 600px; height: auto; margin-bottom: 8px; border-radius: 16px;" />
...
工具が加工原点にあるとき、XY0を設定するためにクリックします。
...
Z軸をゆっくりと下げます。工具がワークピースに接触したとき、Z0を設定するためにクリックします。
...
> ⚠️ **原点設定の注意事項**：
> 
> - この場合、NCプログラムのZ0は四角いワークピースの角のエッジにあるため、Z0を設定するときは、角を真上に調整する必要があります
> - 回転軸加工の前に、ツールパスを設計する際に、まずワークピースをクランプすることをお勧めします。クランプ位置を注意深く考慮する必要があります。生成されたツールパスは、衝突を防ぐためにチャックジョーを避ける必要があります
> - 同時に、Z軸の加工範囲を事前に決定する必要があり、工具の長さを事前に計算して、工具の長さが不足して加工できなくなるのを避ける必要があります
> 
> {.is-warning}
...
> - 進行状況バーを直接ドラッグすることができます
> 
> {.is-info}

## 7. ファイルを転送して加工を開始する

加工を開始するボタンをクリックして、加工ファイルを機器のSDカードに転送します。

<img src="/eng/himill-d1-d1s/media/hmd1s-rotation-axis-processing/6-点击开始.webp" alt="開始するためにクリック" style="width: 800px; height: auto; margin-bottom: 8px; border-radius: 16px;" />

プロンプトに従って、インジケーターライトが虹色の回転光効果を示すまで待ってから、機器のボタンを2秒間長押しして加工を開始します。

<img src="/eng/himill-d1-d1s/media/hmd1s-rotation-axis-processing/6-接受文件开始加工.webp" alt="ファイルを受け取って加工を開始" style="width: 800px; height: auto; margin-bottom: 8px; border-radius: 16px;" />

> ⚠️ **加工制御の注意事項**：
> 
> - 加工中にボタンを単一クリックすると加工が一時停止します。2秒間長押しすると加工が再開します
> - 安全ドア機能が有効になっている場合は、加工を再開する前にドアを閉めてください
> 
> {.is-warning}
...
> ⚠️ **パラメータ調整の注意事項**：
> 
> - 加工中は主軸速度を下げることはお勧めしません。特に金属加工の場合、負荷の増加による工具の折れを避けるためです
> - 値を変更した後、速度は徐々に増加または減少します。実際の状況に応じて値を徐々に増加または減少させる必要があります
> 
> {.is-warning}