...
> ⚠️ **3Dプローブ使用の安全注意事項**：
>
> - **初めて使用する前に、3Dプローブを安全な位置（例：X100,Y-100,Z-10）に移動し、手動で検出をトリガーして正常に動作することを確認してから使用してください**
...
> - 3DプローブはJOG手動移動（ソフトウェアの移動ボタンをクリック）時にのみアラームをトリガーし、検出コマンドG38.2でエラーが発生した場合もアラームを鳴らします。G0、G1、G2コマンドはアラームをトリガーしないため、ボーダーウォーキングや原点復帰などの操作を行う際は、衝突を避けるために特に注意が必要です
> - **ソフトウェアでは、3Dプローブの異常な原点復帰問題を修正するために新しい判定ロジックが追加されました。そのため、原点または原点付近で検出を行うと警告ポップアップが表示される場合があります。これは正常な動作です。検出中にこのポップアップが表示されると、機器は検出完了後に即座に移動を停止し、検出前の位置に戻りませんが、JOG手動移動は依然として使用可能です。3Dプローブの損傷を避けるために必ず注意して操作してください**
>
> {.is-warning}
>
> **原点付近検出ポップアップの例**：
>
> <img src="/eng/himill-d1-d1s/media/hmd1s-3d-probe-using/零点附近探测弹窗.png" alt="原点付近検出ポップアップ" style="width: 400px; height: auto; margin-bottom: 8px; border-radius: 8px;" />
