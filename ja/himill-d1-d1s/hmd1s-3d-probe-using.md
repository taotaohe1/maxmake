...
> - **ソフトウェアでは、3Dプローブの異常な原点復帰問題に対処するために判定ロジックが追加されています。原点または原点付近で検出を行うと、警告ポップアップが表示される場合があります。これは正常な動作です。検出中にこのポップアップが表示されると、機器は即座に移動を停止しますが、JOG手動移動は依然として使用可能です。3Dプローブの損傷を避けるために注意して操作してください**
>
> {.is-warning}
>
> **原点付近検出ポップアップの例**：
