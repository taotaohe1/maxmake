# MaxmakeLab HiMill D1/D1S 连接

## 1. 自动连接

电脑和设备用数据线连接，在首次打开软件时，可以自动查找HiMill设备并连接。

<img src="/chi/himill-d1-d1s/media/hmd1s-connection/1-自动连接，点击回零.webp" alt="自动连接设备" style="width: 800px; height: auto; margin-bottom: 8px; border-radius: 16px;" />

> 📋 **连接提示**：
> 
> - 确保数据线连接稳定
> - 设备已正常开机
> - 首次连接建议使用自动查找功能
> 
> {.is-info}

## 2. 手动连接

电脑和设备用数据线连接，手动选择设备并连接，可以自动查找也可以手动选择串口。

在自动查找失效时，尝试使用串口号连接。

<img src="/chi/himill-d1-d1s/media/hmd1s-connection/2-手动连接.webp" alt="手动连接设备" style="width: 800px; height: auto; margin-bottom: 8px; border-radius: 16px;" />

> ⚠️ **连接注意事项**：
> 
> - 自动查找失效时，可尝试手动选择串口
> - 确保选择的串口号与设备对应
> - 连接前请确认设备已正确开机
> 
> {.is-warning}

## 3. WiFi连接

通过WiFi连接设备，需要连接设备热点，或者给设备配置网络，在局域网中连接。

>  **网络配置指南**：
> 
> - 详细的网络配置方法请参考 [网络配置说明](./hmd1s-network-configuration.md)
> - 支持局域网连接和热点连接两种方式
> - 配置完成后可通过IP地址连接设备
> 
> {.is-info}

> 🔄 **功能状态**：
> 
> - 目前仅支持IP网络连接功能
> - 其他网络相关功能正在开发中
> 
> {.is-info}