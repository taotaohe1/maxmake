...
> ⚠️ **3D测头使用安全注意事项**：
>
> - **初次使用前，建议先将3D测头移至安全位置（如X100,Y-100,Z-10），手动触发探测确认功能正常后再使用**
...
> - 3D测头在移动前误触会报警
> - 3D测头仅在JOG手动移动（软件中点击移动按钮）时触发报警，探测指令G38.2遇错误时也会报警。G0、G1、G2等指令不会触发报警，因此执行走边框、回零等操作时需特别注意避免碰撞
> - **由于软件修复了三坐标异常回零点问题，新增了判定逻辑，因此在零点或零点附近位置探测时会弹出提示弹窗，这属于正常现象。若探测过程中弹出此弹窗，设备会在探测完成后立即停止移动，且不会返回探测前位置，但仍可进行JOG手动移动，请务必注意操作安全，避免损伤3D测头**
>
> {.is-warning}
...
> **零点附近探测弹窗示例**：
>
> <img src="/chi/himill-d1-d1s/media/hmd1s-3d-probe-using/零点附近探测弹窗.png" alt="零点附近探测弹窗" style="width: 400px; height: auto; margin-bottom: 8px; border-radius: 8px;" />
